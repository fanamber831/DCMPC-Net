...
#            self.cri_seq = pearsonr
        ...
        if train_opt.get('FFT_opt'):
            ...
            fft_pix_cls = getattr(loss_module, pixel_type)
            self.fft_pix = fft_pix_cls(**train_opt['FFT_opt']).to(
                self.device)
        else:
            raise ValueError('pixel loss are None.')
...
#########################################以下是新加的代码###############################################
        if 'pickle_tensor' in data:
            self.pkl_train_input_data = data['pickle_tensor'].to(self.device)
#########################################以上是新加的代码###############################################
        if self.mixing_flag:
...
#        print(x1, x2)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(self.lq,self.pkl_train_input_data )
        ...
        l_fft = self.fft_pix(self.output, self.gt)
        loss_dict['l_fft'] = l_fft
#        l_pred = self.cri_celoss(self.pred, self.label.to(self.pred.device))
#        loss_dict['l_pred'] = l_pred
#        print("pear:", l_pear, "pix:", l_pix)
        loss_total = l_pix + l_pear + l_fft #+ 0.01*l_pred#+ l_mask

        # lambda_l2 = 0.0
        # loss_total = loss_total + lambda_l2 * sum(p.sum() for p in self.net_g.parameters())

        loss_total.backward()
        
        if self.opt['train']['use_grad_clip']:
            # torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), 0.01, error_if_nonfinite=False)            # 原始代码
            torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), 0.01)            # 原始代码
    ...
    def nonpad_test(self, img=None, text_tensor=None):
        if img is None:
            img = self.lq
################################################以下是新加的代码################################################
        if text_tensor is None:
            text_tensor = self.pkl_train_input_data
################################################以上是新加的代码################################################
        if hasattr(self, 'net_g_ema'):
            ...
            self.net_g.eval()
            with torch.no_grad():
                # pred = self.net_g(img)                                 # 原来的
                pred = self.net_g(img,text_tensor)          # 添加的
            if isinstance(pred, list):
                pred = pred[-1]
            self.output = pred
            self.net_g.train()
